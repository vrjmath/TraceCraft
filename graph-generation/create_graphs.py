...
### load datasets
    ...
    # real graphs
        ...
        graphs = graphs[0:200]
        args.max_prev_node = 15
    elif args.graph_type == 'traces':

        # Load your saved data
        data = torch.load('../data_files/traces_processed/train.pth', map_location='cpu')
        ...
        for x_n, src, dst in zip(x_n_list, src_list, dst_list):
            G = nx.Graph()
            G.add_nodes_from(range(x_n.shape[0]))  # Add all nodes
            edge_list = list(zip(src.tolist(), dst.tolist()))
            G.add_edges_from(edge_list)

            # Optionally store node features if needed
            for i in range(x_n.shape[0]):
                G.nodes[i]['feat'] = x_n[i].numpy()

            graphs.append(G)

        # Set based on your data stats (or dynamically)
        args.max_prev_node = max([G.number_of_nodes() for G in graphs]) - 1
