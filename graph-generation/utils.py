...
def pick_connected_component_new(G):
    adj_list = [list(G.neighbors(n)) for n in sorted(G.nodes())]
    ...
    G_sub = G.subgraph(node_list).copy()
    ccs = (G_sub.subgraph(c).copy() for c in nx.connected_components(G_sub))
    G_largest = max(ccs, key=len)
    return G_largest
...
def load_graph_list(fname,is_real=True):
    ...
    for i in range(len(graph_list)):
        ...
        if is_real:
            ccs = (graph_list[i].subgraph(c).copy() for c in nx.connected_components(graph_list[i]))
            graph_list[i] = max(ccs, key=len)
